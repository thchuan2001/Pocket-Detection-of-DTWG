    ...
    def evaluate(self, generated: list[dict], bad_case_dir: str = None, good_case_dir: str = None, output_dir: str = None):
        ...
        for idx, res in enumerate(results):
            ...
            if output_dir is not None:
                ...
                mol = res['mol']
                if "APObind" in ligand_filename:
                    case_name=os.path.basename(ligand_filename).replace(".pdb","")
                else :
                    if "/" in ligand_filename:
                        case_name=os.path.join(
                            os.path.dirname(ligand_filename),
                            os.path.basename(ligand_filename)[:10]).replace("/","__")
                    else:
                        case_name=ligand_filename
                random_idx=str(random.randint(0,100000))
                with Chem.SDWriter(os.path.join(output_dir, f'{case_name}_{random_idx}.sdf')) as w:
                    w.write(mol)
                    ...
                    if good_case_dir is not None:
                        ...
                        if "APObind" in ligand_filename:
                            ...
                            case_name=os.path.join(
                                os.path.dirname(ligand_filename),
                                os.path.basename(ligand_filename)[:10]).replace("/","__")
                        with Chem.SDWriter(os.path.join(good_case_dir, f'{case_name}_{idx}.sdf')) as w:
                            w.write(mol)
