...
class SBDDTrainLoop(pl.LightningModule):
    ...
    def shared_sampling_step(self, batch, batch_idx, sample_num_atoms, desc=''):
        ...
        elif sample_num_atoms == 'ref_random':
            batch_ligand = batch.ligand_element_batch
            ligand_num_atoms = scatter_sum(torch.ones_like(batch_ligand), batch_ligand, dim=0).to(ligand_pos.device)
            noise = torch.randint(-10,10,ligand_num_atoms.size()).to(ligand_pos.device)
            ligand_num_atoms = ligand_num_atoms + noise
            ligand_num_atoms = torch.clamp(ligand_num_atoms, min=3).to(ligand_pos.device)
            batch_ligand = torch.repeat_interleave(torch.arange(len(batch)).to(ligand_pos.device), torch.tensor(ligand_num_atoms).to(ligand_pos.device)).to(ligand_pos.device)
        else:
