...
def get_dataloader_from_pdb(cfg):
    assert cfg.evaluation.pocket_path is not None
    pocket_path = cfg.evaluation.pocket_path
    pocket_files = glob.glob(pocket_path+"/*.pdb")[:11]
    pocket_list=[]
    for pocket_fn in pocket_files:
        pocket = PDBProtein(pocket_fn)
        pocket_dict = pocket.to_dict_atom()
        ligand_dict = parse_sdf_file("./ch4.sdf")

        data = ProteinLigandData.from_protein_ligand_dicts(
            protein_dict=torchify_dict(pocket_dict),
            ligand_dict=torchify_dict(ligand_dict),
        )
        data.protein_filename = os.path.basename(pocket_fn)
        data.ligand_filename = os.path.basename(pocket_fn)
    ...
    test_set = [transform(data) for data in pocket_list]
    new_test_set = []
    for data in test_set:
        new_test_set = new_test_set + [data] * cfg.evaluation.num_samples
    ...
    cfg.evaluation.docking_config.protein_root = None #os.path.dirname(os.path.abspath(protein_fn))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pocket_path", type=str, default=None)
    parser.add_argument("--docking_mode", type=str, default=None, choices=['vina_score', 'vina_dock'])
    parser.add_argument("--output_dir", required=True)

    _args = parser.parse_args()
    cfg = Config(**_args.__dict__)
    cfg.evaluation.pocket_path=_args.pocket_path
    cfg.output_dir = _args.output_dir
