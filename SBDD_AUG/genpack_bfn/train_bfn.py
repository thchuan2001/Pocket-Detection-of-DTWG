...
import glob
from format_output_sdf import format_output
...
if __name__ == "__main__":
    ...
    # eval params
    parser.add_argument('--ckpt_path', type=str, default='/data/bfn_data/ckpts/molcraft_offical.ckpt', help='path to the checkpoint')
    parser.add_argument("--num_samples", type=int)
    parser.add_argument("--sample_steps", type=int, default=100)
    parser.add_argument('--sample_num_atoms', type=str, default='prior', choices=['prior', 'ref',"ref_random"])
    ...
    parser.add_argument("--inference_input_pkl", type=str, default=None)
    parser.add_argument("--device", type=str, default='0')

    _args = parser.parse_args()
    device_list=[int(x) for x in _args.device.split(',')]
    print ("Using device list: ", device_list)
    if _args.test_only:
        _args.no_wandb = True
    ...
    if _args.output_dir and not os.path.exists(_args.output_dir):
        os.makedirs(_args.output_dir, exist_ok=True)

    if _args.output_dir :
        give_permission(_args.output_dir)
    ...
    cfg.data.inference_input_pkl = _args.inference_input_pkl
    ...
    if _args.output_dir is not None:
        cfg.output_dir = os.path.join(_args.output_dir, "tmp_dir")
        os.makedirs(cfg.output_dir, exist_ok=True)      
        print("############# [rank", trainer.global_rank, "] set output_dir to", cfg.output_dir, "#############")
    ...
    print("##### rank", trainer.global_rank, "finished #####")
    
    if dist.is_initialized():
        dist.barrier()

    # convert output_dir to sdf_dir
    if cfg.test_only and trainer.global_rank == 0:
        print("start to format at rank 0")
        if _args.output_dir is not None:
            format_output(cfg.output_dir,_args.output_dir, max_ligands=num_samples)
